...
if (isset($_GET['id'])) {
    ...
    $sqlQuery = sprintf("SELECT `id`, `risk_name`, `risk_id`, `risk_type`, `risk_sum`, `risk_premium` FROM `risk_cache` WHERE `contract_id` = %d ORDER BY `risk_name` ASC;", $_GET['id']);
    ...
    if ($res) {
        ...
        while($w = $res->GetNext()){
            ...
            $cols[] = sprintf('<td class="center"><button risktype="%s" id="%d" ibid="%d" class="remove-risk btn btn-danger">X</button></td>', $riskType, $w['id'], $w['risk_id']);
            $rows[] = sprintf('<tr class="risk">%s</tr>', implode('',$cols));
            $sum += floatval($w['risk_sum']);
        }

        $mandatory = (empty($fixedArr)) ? 'Y' : 'N';
        if ($riskType == 'eg' or $riskType == 'ug') {
            $mandatory = 'Y';
        }
